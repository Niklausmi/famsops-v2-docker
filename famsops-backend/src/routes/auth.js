...
router.post('/login', async (req, res) => {
  ...
  const { rows } = await query(
    `SELECT
       u.id, u.user_id, u.name, u.email, u.password_hash,
       u.role, u.role_id, u.department, u.phone, u.active,
       u.avatar, u.technician_id, u.last_login,
       r.name  AS role_name,
     ...
     WHERE u.email = $1
     GROUP BY u.id, u.user_id, u.name, u.email, u.password_hash,
              u.role, u.role_id, u.department, u.phone, u.active,
              u.avatar, u.technician_id, u.last_login,
  ...
  res.json({
    ...
    user: {
      ...
      permissions: user.permissions || [],   // array sent to frontend
    },
  });
});
...
router.get('/me', auth, async (req, res) => {
  const { rows } = await query(
    `SELECT u.user_id, u.name, u.email, u.role, u.role_id,
            u.department, u.phone, u.avatar, u.last_login,
     ...
     LEFT JOIN permissions p      ON p.id  = rp.permission_id
     WHERE u.user_id = $1
     GROUP BY u.id, u.user_id, u.name, u.email, u.role, u.role_id, u.department, u.phone, u.active, u.avatar, u.technician_id, u.last_login, r.name, r.label, r.color`,
    [req.user.userId]
  );
...
// Returns the full roles + permissions matrix (admin/management only)
router.get('/permissions', auth, can('users', 'read'), async (req, res) => {
  const { rows: roles } = await query(
    `SELECT r.*,
       COALESCE(
         json_agg(json_build_object('module', p.module, 'action', p.action, 'label', p.label, 'id', p.id))
         FILTER (WHERE p.id IS NOT NULL),
         '[]'
       ) AS permissions
     FROM roles r
...
// Grant/revoke a permission for a role (admin only)
router.patch('/role-permissions', auth, can('users', 'update'), async (req, res) => {
  ...
  res.json({ message: grant ? 'Permission granted' : 'Permission revoked', roleId, permissionId, grant });
});
